...
function isPalindrome(str) {
  const stroka = str
    ...
    .toLowerCase()
    .replace(/[^a-zA-Z0-9]/g, '');
  return stroka === str.toLowerCase().replace(/[^a-zA-Z0-9]/g, '');
}
...
function findLongestWord(sentence) {
  const longestWord = sentence.split(' ').sort(function longWord(a, b) {
...
function unbracketTag(str) {
  ...
  let pos = str.indexOf('<');
  if (pos !== -1) {
    stroka = str.slice(0, pos) + str.slice(pos + 1);
  }
  pos = stroka.indexOf('>');
  if (pos !== -1) {
    stroka = stroka.slice(0, pos) + stroka.slice(pos + 1);
  }
  return stroka;
}
...
function getCardId(value) {
  ...
  return card.indexOf(value[0]) + suit.indexOf(value.slice(-1)) * 13;
}
